...
import { relatorioService, resolveRelatorioApiError } from '../../services/relatorioService';
...
function isActivePending(status?: string, stale?: boolean): boolean {
  return status === 'PENDENTE' && !stale;
}
...
export function Relatorios() {
  ...
  const hasPending =
    isActivePending(relatorioFolha?.status, relatorioFolha?.stale)
    || isActivePending(relatorioBeneficio?.status, relatorioBeneficio?.stale);
  ...
  const gerarRelatorioFolha = async () => {
    if (isActivePending(relatorioFolha?.status, relatorioFolha?.stale)) return;
    try {
      ...
      showNotification(resolveRelatorioApiError(error), 'error');
    } finally {
  ...
  const gerarRelatorioBeneficio = async () => {
    if (isActivePending(relatorioBeneficio?.status, relatorioBeneficio?.stale)) return;
    try {
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <RelatorioCatalogCard
            ...
            stale={relatorioBeneficio?.stale}
