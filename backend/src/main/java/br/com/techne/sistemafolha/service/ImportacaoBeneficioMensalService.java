...
import br.com.techne.sistemafolha.dto.ImportacaoResultadoDTO;
import br.com.techne.sistemafolha.exception.BeneficioMensalDuplicadaException;
import br.com.techne.sistemafolha.exception.ImportacaoBeneficioMensalInvalidaException;
...
public class ImportacaoBeneficioMensalService {
    ...
    public ImportacaoResultadoDTO importar(
        ...
        logger.info("Iniciando importação de benefícios mensais - Arquivo: {}, competência: {} a {}",
                arquivo.getOriginalFilename(), competenciaInicio, competenciaFim);
        ...
        try (Workbook workbook = WorkbookFactory.create(arquivo.getInputStream())) {
            ...
            for (int rowIndex = PRIMEIRA_LINHA_DADOS; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
                ...
                Funcionario funcionario = funcionarioRepository.findByCpfAndAtivoTrue(cpf).orElse(null);
                if (funcionario == null) {
        ...
        logger.info("Importação concluída - processadas: {}, total: {}",
                registros.size(), totalValor);
    ...
    private void substituirRegistrosExistentes(LocalDate competenciaInicio, LocalDate competenciaFim) {
        logger.info("Removendo registros existentes (ativos e inativos) para competência {} a {}",
                competenciaInicio, competenciaFim);
