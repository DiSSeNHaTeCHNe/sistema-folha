...
Canonical state:  _docs/specs/lessons.json   (machine-owned — do NOT hand-edit)
Rendered view:    _docs/specs/LESSONS.md      (regenerated on every write)

Pure standard library. No dependencies. Run from the project root (the dir that
contains _docs/specs), or pass --root.
...
STORE_REL = os.path.join("_docs", "specs", "lessons.json")
RENDER_REL = os.path.join("_docs", "specs", "LESSONS.md")
...
def _save(root, data):
    os.makedirs(os.path.join(root, "_docs", "specs"), exist_ok=True)
...
def _render(root, data):
    ...
    lines.append("> Canonical state lives in `_docs/specs/lessons.json`. Edit lessons only via the script.")
...
def main(argv=None):
    ...
    p.add_argument("--root", default=".", help="Project root containing _docs/specs/ (default: current dir)")
